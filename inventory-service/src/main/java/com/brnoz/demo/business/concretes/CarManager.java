...
public class CarManager implements CarService {
	...
	public DataResult<List<GetAllCarResponse>> getAll() {
		...
		List<GetAllCarResponse> responses = cars.stream()
				.map(car -> this.mapperService.forResponse().map(car, GetAllCarResponse.class))
	...
	public DataResult<GetByCarResponse> getByCar(String plateNumber) {
		this.rules.checkIfExistByPlateNumber(plateNumber);
		Car car = this.repository.findByPlateNumber(plateNumber);
		GetByCarResponse response = this.mapperService.forResponse().map(car, GetByCarResponse.class);
		return new SuccessDataResult<GetByCarResponse>(response, Messages.CarListed);
	}

	@Override
	public Result delete(UUID id) {
		this.rules.checkIfExistByCarId(id);
